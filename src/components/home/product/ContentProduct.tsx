...
import SkeletonProduct from '@/components/skeleton/product/SkeletonProduct';
...
export default function ContentProduct( props: Props ) {
    ...
    if(isLoadingGetProductByCategory) {
        return (
            <div>
                <SkeletonProduct />
            </div>
    ...
    const lengthDataProduct = dataProduct.length
    if(lengthDataProduct < 5) return null;
    
    const handlePrev = () => {
        if(startIndex + visibleCount > 5) {
            setStartIndex(prev => prev - 1)
        }
    }

    const handleNext = () => {
        if(startIndex + visibleCount < lengthDataProduct) {
            setStartIndex(prev => prev + 1)
        }
    ...
    return (
        <div className='relative'>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 no-scrollbar gap-3 pt-2 mx-auto">
                {dataProduct.slice(startIndex,startIndex + visibleCount).map((prod) => (
                    <Card
                        key={prod.id}
    ...
    {/* 
            ...
            {dataProduct.length > 5 && (
                <div>
                    ...
                    {(startIndex + visibleCount) < lengthDataProduct &&
                        <SliderButton
                            direction="next"
                            onClick={handleNext}
                        />
